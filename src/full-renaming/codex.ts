...
const checkMaxTokens = (text: string, maxTokens: number) =>
  assert(
    countTokens(text) <= maxTokens,
    `Too many tokens in prompt: ${countTokens(text)}/${maxTokens}`,
  );
...
###`;
    ...
    if (choices) {
      ...
      if (firstSuggestedText) {
        fineTuneLogger.debug(
          `Suggestion for call ${fineTuneId}:\n${firstSuggestedText}`,
        );
      }
    }
...
const getChatSuggests:TextSuggester =
  async (task) => {
    ...
    const chat: ChatCompletionRequestMessage[] = [
      {
        role: 'system',
        content: `You will be shown a code snippet with some randomized variables.
Some of the variable names have been randomized.
Try to recover the original variable names.`,
      },
      {
        role: 'user',
        content: `Code:
...
vuxn2 -> base`,
      },
      {
        ...
        content: `Code:
${task.code}
Variables: ${targetList}`,
      },
    ];

    const numTokens = countTokens(JSON.stringify(chat));
    const model =
      numTokens > bigChatThreshold ? `${baseChatModel}-16k` : baseChatModel;
    checkMaxTokens(JSON.stringify(chat), maxChatTokens);
    ...
    assert(message, 'ChatGPT returned empty response');
